...
//add by liuhy
func WithKeyRequest(alg string, size int) CAClientOption {
	return func(o *caClientOption) error {
		o.keyRequest = &KeyRequest{
			Algo: alg,
			Size: size,
		}
		return nil
...
func NewCAClient(orgName string, ctx contextApi.Client, opts ...CAClientOption) (*CAClientImpl, error) {
	...
	adapter, err := newFabricCAAdapter(ctx.InfraProvider().IsGMTLS(), caID, ctx.CryptoSuite(), ctx.IdentityConfig())
	if err != nil {
	...
	mgr := &CAClientImpl{
		...
		keyRequest:      options.keyRequest, //add by liuhy for alg ca
	}
	return mgr, nil
}
...
func (c *CAClientImpl) Enroll(request *api.EnrollmentRequest) error {
	...
	//add by liuhy for alg ca
	if request.CSR == nil && c.keyRequest != nil {
		request.CSR = &api.CSRInfo{
			KeyRequest: &api.KeyRequest{
				Algo: c.keyRequest.Algo,
				Size: c.keyRequest.Size,
			},
		}
	}
...
func (c *CAClientImpl) getRegistrar(enrollID string, enrollSecret string) (msp.SigningIdentity, error) {
	...
	if err != nil {
		...
		err = c.Enroll(&api.EnrollmentRequest{Name: enrollID, Secret: enrollSecret, CSR: csr})
		if err != nil {
			return nil, err
		}
		registrar, err = c.identityManager.GetSigningIdentity(enrollID)
