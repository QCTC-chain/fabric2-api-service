...
func newFabricCAAdapter(isGMTLS bool, caID string, cryptoSuite core.CryptoSuite, config msp.IdentityConfig) (*fabricCAAdapter, error) {

	caClient, err := createFabricCAClient(isGMTLS, caID, cryptoSuite, config)
	if err != nil {
		return nil, err
	}
...
func createCSRInfo(csr *api.CSRInfo) *caapi.CSRInfo {
	...
	//add by liuhy for ca 优化掉环境变量
	if csr.KeyRequest != nil {
		csrInfo.KeyRequest = &caapi.KeyRequest{
...
func createFabricCAClient(isGMTLS bool, caID string, cryptoSuite core.CryptoSuite, config msp.IdentityConfig) (*calib.Client, error) {
