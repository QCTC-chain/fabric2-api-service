...
func (c *Ledger) QueryInfo(reqCtx reqContext.Context, targets []fab.ProposalProcessor, verifier ResponseVerifier) ([]*fab.BlockchainInfoResponse, error) {
	...
	tprs, errs := queryChaincode(reqCtx, c.chName, cir, targets, verifier, c.EnableTxTimeStamp)
...
func (c *Ledger) QueryConfigBlock(reqCtx reqContext.Context, targets []fab.ProposalProcessor, verifier ResponseVerifier) (*common.Block, error) {
	...
	tprs, err := queryChaincode(reqCtx, c.chName, cir, targets, verifier, c.EnableTxTimeStamp)
...
func queryChaincode(reqCtx reqContext.Context, channelID string, request fab.ChaincodeInvokeRequest, targets []fab.ProposalProcessor, verifier ResponseVerifier, enableTxTimeStamp bool) ([]*fab.TransactionProposalResponse, error) {
	...
	opt := fab.WithTxTimeStamp(enableTxTimeStamp)
	txh, err := txn.NewHeader(ctx, channelID, opt)
	if err != nil {
