...
func NewTransactor(reqCtx reqContext.Context, cfg fab.ChannelCfg, enableTxTimeStamp bool) (*Transactor, error) {
...
func orderersFromChannelCfg(ctx context.Client, cfg fab.ChannelCfg) ([]fab.Orderer, error) {
	...
	for _, target := range cfg.Orderers() {
		...
		// jzk modify,20210426:将entityMatcher消掉的前缀给加回来，否则solo、不开tls时，cc实例化不成功
		//isGRPC := true
		//for _, v := range ordererDict{
		//	if strings.Contains(v.URL,"grpcs://"){
		//		isGRPC = false
		//	}
		//	break
		//}
		//if isGRPC{
		//	if !strings.Contains(oCfg.URL,"grpc://"){
		//		oCfg.URL = "grpc://" + oCfg.URL
		//	}
		//}else{
		//	if !strings.Contains(oCfg.URL,"grpcs://"){
		//		oCfg.URL = "grpcs://" + oCfg.URL
		//	}
		//}

		// jzk modify, 20211125: 适配config文件entitymatchers.urlSubstitutionExp未添加协议栈（grpc://  grpcs://）问题
		isGRPCS := false
		if oCfg.TLSCACert != nil && oCfg.TLSCACert.Raw != nil && len(oCfg.TLSCACert.Raw) > 0{
			isGRPCS = true
		}
		...
		o, err := ctx.InfraProvider().CreateOrdererFromConfig(ctx.InfraProvider().IsGMTLS(), &oCfg)
		if err != nil {
...
func orderersFromChannel(ctx context.Client, channelID string) ([]fab.Orderer, error) {
	...
	for _, chOrderer := range chNetworkConfig.Orderers {
		...
		orderer, err := ctx.InfraProvider().CreateOrdererFromConfig(ctx.InfraProvider().IsGMTLS(), ordererConfig)
		if err != nil {
			return nil, errors.WithMessage(err, "failed to create orderer from config")
		}
...
func (t *Transactor) CreateTransactionHeader(opts ...fab.TxnHeaderOpt) (fab.TransactionHeader, error) {
	...
	opt := fab.WithTxTimeStamp(t.enableTxTimeStamp)
