...
func (c *EndpointConfig) loadEndpointConfigEntities(configEntity *endpointConfigEntity) error {
	...
	// jzk modify, 20211125
	isGRPCs:= c.judgeGRPCs(configEntity)
	matchError := c.compileMatchersForScheme(isGRPCs)
	if matchError != nil {
		return matchError
	}
...
// jzk modify, 20211125: 适配config文件entitymatchers.urlSubstitutionExp未添加协议栈（grpc://  grpcs://）问题
func (c *EndpointConfig) judgeGRPCs(configEntity *endpointConfigEntity) bool{
	for _, peer := range configEntity.Peers{
		if strings.Contains(peer.URL,"grpcs://"){
			return true
		}
	}
	for _, orderer := range configEntity.Orderers{
		if strings.Contains(orderer.URL,"grpcs://"){
			return true
		}
	}
...
func (c *EndpointConfig) compileMatchersForScheme(isGRPCs bool) error {
	...
	if entMatchers.matchers["peer"] != nil && len(entMatchers.matchers["peer"]) > 0 {
		for _, matcher := range entMatchers.matchers["peer"]{
			if !strings.Contains(matcher.URLSubstitutionExp,"grpc://") && !strings.Contains(matcher.URLSubstitutionExp,"grpcs://"){
				if isGRPCs{
					logger.Infof("replace entitymatchers.peer.urlSubstitutionExp from %s to %s",matcher.URLSubstitutionExp,"grpcs://" + matcher.URLSubstitutionExp)
					matcher.URLSubstitutionExp = "grpcs://" + matcher.URLSubstitutionExp
				}else{
					logger.Infof("replace entitymatchers.peer.urlSubstitutionExp from %s to %s",matcher.URLSubstitutionExp,"grpc://" + matcher.URLSubstitutionExp)
					matcher.URLSubstitutionExp = "grpc://" + matcher.URLSubstitutionExp
				}
			}
		}
	}

	if entMatchers.matchers["orderer"] != nil && len(entMatchers.matchers["orderer"]) > 0 {
		for _, matcher := range entMatchers.matchers["orderer"]{
			if !strings.Contains(matcher.URLSubstitutionExp,"grpc://") && !strings.Contains(matcher.URLSubstitutionExp,"grpcs://"){
				if isGRPCs{
					logger.Infof("replace entitymatchers.orderer.urlSubstitutionExp from %s to %s",matcher.URLSubstitutionExp,"grpcs://" + matcher.URLSubstitutionExp)
					matcher.URLSubstitutionExp = "grpcs://" + matcher.URLSubstitutionExp
				}else{
					logger.Infof("replace entitymatchers.orderer.urlSubstitutionExp from %s to %s",matcher.URLSubstitutionExp,"grpc://" + matcher.URLSubstitutionExp)
					matcher.URLSubstitutionExp = "grpc://" + matcher.URLSubstitutionExp
				}
			}
		}
