...
func (c *client) Send(isGMTLS bool, ctx context.Context, req *Request, targets ...fab.PeerConfig) (<-chan Response, error) {
	...
	for _, t := range targets {
		...
		go func(target fab.PeerConfig) {
			...
			discoveryResponse, err := c.send(isGMTLS, ctx, req.r, target)
...
func (c *client) send(isGMTLS bool, reqCtx context.Context, req *discclient.Request, target fab.PeerConfig) (discclient.Response, error) {
	...
	if isGMTLS {
		opts = append(opts, comm.WithInGMTLS())
	}
