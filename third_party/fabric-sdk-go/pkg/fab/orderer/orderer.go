...
func New(config fab.EndpointConfig, opts ...Option) (*Orderer, error) {
	...
	if endpoint.AttemptSecured(orderer.url, orderer.allowInsecure) {
		...
		tlsConfig, err := comm.TLSConfig(orderer.tlsCACert, orderer.serverName, config, orderer.inGMTLS)
		if err != nil {
			return nil, err
		}
...
func (o *Orderer) conn(ctx reqContext.Context) (*grpc.ClientConn, error) {
	...
	// jzk
	ctx2 := reqContext.WithValue(ctx, "servername", o.serverName)
	return commManager.DialContext(ctx2, o.url, o.grpcDialOption...)
