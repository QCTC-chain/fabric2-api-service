...
import (
	...
	gmcredentials "gitee.com/china_uni/tjfoc-gm/tls/gmcredentials"
...
func newPeerEndorser(endorseReq *peerEndorserRequest) (*peerEndorser, error) {
	...
	if endpoint.AttemptSecured(endorseReq.target, endorseReq.allowInsecure) {
		...
		tlsConfig, err := comm.TLSConfig(endorseReq.certificate, endorseReq.serverHostOverride, endorseReq.config, endorseReq.enableGMTLS)
		if err != nil {
			return nil, err
		}
		// jzk
...
func (p *peerEndorser) conn(ctx reqContext.Context) (*grpc.ClientConn, error) {
	...
	ctx2 := reqContext.WithValue(ctx, "servername", p.serverName)
	return commManager.DialContext(ctx2, p.target, p.grpcDialOption...)
}
