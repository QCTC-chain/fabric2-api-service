...
func New(channelID string, enableTxTimeStamp bool, options ...Option) (*ChannelConfig, error) {
	...
	return &ChannelConfig{channelID: channelID, opts: opts, enableTxTimeStamp: enableTxTimeStamp}, nil
}
...
func (c *ChannelConfig) queryBlockFromPeers(reqCtx reqContext.Context) (*common.Block, error) {
	...
	l, err := channel.NewLedger(c.channelID,c.enableTxTimeStamp)  // // jzk, tx with timestamp, for fabric 1.4.8-enhanced
	if err != nil {
...
func (c *ChannelConfig) calculateTargetsFromConfig(ctx context.Client) ([]fab.ProposalProcessor, error) {
	...
	for _, p := range chPeers {
		...
		newPeer, err := ctx.InfraProvider().CreatePeerFromConfig(ctx.InfraProvider().IsGMTLS(), (&p.NetworkPeer))
...
func (c *ChannelConfig) queryBlockFromOrderer(reqCtx reqContext.Context) (*common.Block, error) {
	...
	return resource.LastConfigFromOrderer(reqCtx, c.channelID, c.opts.Orderer, resource.WithRetry(c.opts.RetryOpts),resource.WithTxTimeStamp(c.enableTxTimeStamp))
}
