...
func (cc *CachingConnector) DialContext(ctx context.Context, target string, opts ...grpc.DialOption) (*grpc.ClientConn, error) {
	...
	// jzk modify, 20210426: for invalid connection
	cc.lock.Lock()
	if c.conn.GetState() == connectivity.TransientFailure {
		// log.Println("jzk modify: conn is TransientFailure, recreate it")
		createdConn, err := cc.createConn(ctx, target, opts...)
		if err != nil {
			cc.lock.Unlock()
			return nil, errors.WithMessage(err, "connection creation failed")
...
func (cc *CachingConnector) createConn(ctx context.Context, target string, opts ...grpc.DialOption) (*cachedConn, error) {
	...
	//cconn, ok := cc.loadConn(servername)
	//if ok {
	//	return cconn, nil
	//}

	// jzk, 20240624, 修复 conn TransientFailure
	cconn, ok := cc.loadConn(servername)
	if ok && cconn.conn.GetState() == connectivity.Ready {
		return cconn, nil
	}
