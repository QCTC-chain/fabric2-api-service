...
func WithInGMTLS() options.Opt {
	return func(p options.Params) {
		if setter, ok := p.(inGMTLSSetter); ok {
			setter.SetInGMTLS(true)
		}
	}
}
...
//add by liuhy for gm tls
func (p *params) SetInGMTLS(value bool) {
	logger.Debugf("InGMTLS: %t", value)
	p.inGMTLS = value
}
...
//add by liuhy for gm tls
type inGMTLSSetter interface {
	SetInGMTLS(value bool)
}
