...
func NewConnection(ctx fabcontext.Client, url string, opts ...options.Opt) (*GRPCConnection, error) {
	...
	ctx2 := fmtctx.WithValue(reqCtx, "servername", params.hostOverride)
	grpcconn, err := commManager.DialContext(ctx2, endpoint.ToAddress(url), dialOpts...)
	if err != nil {
...
func newDialOpts(config fab.EndpointConfig, url string, params *params) ([]grpc.DialOption, error) {
	...
	if endpoint.AttemptSecured(url, params.insecure) {
		...
		tlsConfig, err := comm.TLSConfig(params.certificate, params.hostOverride, config, params.inGMTLS)
		if err != nil {
			return nil, err
		}
