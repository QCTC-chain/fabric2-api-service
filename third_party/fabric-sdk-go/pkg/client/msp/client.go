...
func initClientFromOptions(clientProvider context.ClientProvider, opts ...ClientOption) (fab.ClientContext, *Client, error) {
	...
	c := Client{
		...
		orgName:    o.orgName,
		caID:       o.caID,
		defUserAlg: o.defUserAlg, //add by liuhy for alg ca
	}
...
func newCAClient(ctx context.Client, orgName string, caID string, keyReq *KeyRequest) (mspapi.CAClient, error) {
	var (
		caClient *msp.CAClientImpl
		err      error
	)

	//modify by liuhy for mult alg for gen keys
	if keyReq != nil {
		caClient, err = msp.NewCAClient(orgName, ctx, msp.WithCAInstance(caID), msp.WithKeyRequest(keyReq.Algo, keyReq.Size))
		if err != nil {
...
func (c *Client) CreateIdentity(request *IdentityRequest) (*IdentityResponse, error) {
	//modify by liuhy for mutl algs
	ca, err := newCAClient(c.ctx, c.orgName, c.caID, c.defUserAlg)
	if err != nil {
		return nil, err
	}
...
func (c *Client) ModifyIdentity(request *IdentityRequest) (*IdentityResponse, error) {
	//modify for liuhy for mult algs
	ca, err := newCAClient(c.ctx, c.orgName, c.caID, c.defUserAlg)
	if err != nil {
		return nil, err
	}
...
func createCSRInfo(csr *CSRInfo) *mspapi.CSRInfo {
	...
	//add by liuhy for mult algs
	var csrInfo *mspapi.CSRInfo
	if csr.KeyRequest != nil {
		csrInfo = &mspapi.CSRInfo{
			CN:    csr.CN,
			Hosts: csr.Hosts,
			KeyRequest: &mspapi.KeyRequest{
