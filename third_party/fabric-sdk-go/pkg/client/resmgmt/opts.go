...
func WithTargetEndpoints(keys ...string) RequestOption {
	return func(ctx context.Client, opts *requestOptions) error {
		...
		for _, url := range keys {
			...
			peer, err := ctx.InfraProvider().CreatePeerFromConfig(ctx.InfraProvider().IsGMTLS(), peerCfg)
			if err != nil {
...
func WithOrdererEndpoint(key string) RequestOption {
	...
	return func(ctx context.Client, opts *requestOptions) error {
		...
		//modify by liuhy for gm tls
		orderer, err := ctx.InfraProvider().CreateOrdererFromConfig(ctx.InfraProvider().IsGMTLS(), ordererCfg)
		if err != nil {
