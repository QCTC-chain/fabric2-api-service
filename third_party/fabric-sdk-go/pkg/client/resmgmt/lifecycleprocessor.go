...
func newLifecycleProcessor(ctx context.Client, targetProvider targetProvider, signatureVerifier signatureVerifier, committer committer, enableTxTimeStamp bool) *lifecycleProcessor {
	return &lifecycleProcessor{
		lifecycleResource:    resource.NewLifecycle(enableTxTimeStamp),
		ctx:                  ctx,
		getCCProposalTargets: targetProvider,
		verifyTPSignature:    signatureVerifier,
		commitTransaction:    committer,
...
func (p *lifecycleProcessor) prepare(reqCtx reqContext.Context, channelID string, opts requestOptions) ([]fab.Peer, fab.ChannelService, fab.Transactor, *txn.TransactionHeader, error) {
	...
	// jzk, tx with timestamp, for fabric 1.4.8-enhanced
	opt := fab.WithTxTimeStamp(p.enableTxTimeStamp)

	txh, err := txn.NewHeader(p.ctx, channelID, opt)
	if err != nil {
