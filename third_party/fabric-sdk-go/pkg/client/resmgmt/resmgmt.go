...
func New(ctxProvider context.ClientProvider, opts ...ClientOption) (*Client, error) {
	...
	// jzk, tx with timestamp, for fabric 1.4.8-enhanced
	// set in opts, or use setting in sdk
	resourceClient.enableTxTimeStamp = resourceClient.enableTxTimeStamp || ctx.InfraProvider().EnableTxTimeStamp()
	...
	// jzk, tx with timestamp, for fabric 1.4.8-enhanced
	resourceClient.lifecycleProcessor = newLifecycleProcessor(ctx, resourceClient.getCCProposalTargets, resourceClient.verifyTPSignature, resourceClient.sendTransactionAndCheckEvent, resourceClient.enableTxTimeStamp)
	resourceClient.lifecycleProcessor.enableTxTimeStamp = resourceClient.enableTxTimeStamp
...
func (rc *Client) JoinChannel(channelID string, options ...RequestOption) error {
	...
	err = resource.JoinChannel(peerReqCtx, joinChannelRequest, peersToTxnProcessors(targets), resource.WithRetry(opts.Retry), resource.WithTxTimeStamp(rc.enableTxTimeStamp)) // jzk, tx with timestamp, for fabric 1.4.8-enhanced
	if err != nil {
...
func (rc *Client) sendInstallCCRequest(req InstallCCRequest, reqCtx reqContext.Context, newTargets []fab.Peer, responses []InstallCCResponse) ([]InstallCCResponse, error) {
	...
	transactionProposalResponse, _, err := resource.InstallChaincode(reqCtx, icr, peer.PeersToTxnProcessors(newTargets), resource.WithTxTimeStamp(rc.enableTxTimeStamp)) // jzk, tx with timestamp, for fabric 1.4.8-enhanced
	if err != nil {
...
func (rc *Client) QueryInstalledChaincodes(options ...RequestOption) (*pb.ChaincodeQueryResponse, error) {
	...
	return resource.QueryInstalledChaincodes(reqCtx, opts.Targets[0], resource.WithRetry(opts.Retry), resource.WithTxTimeStamp(rc.enableTxTimeStamp)) // jzk, tx with timestamp, for fabric 1.4.8-enhanced
}
...
func (rc *Client) createTP(req InstantiateCCRequest, channelID string, ccProposalType chaincodeProposalType) (*fab.TransactionProposal, fab.TransactionID, error) {
	...
	// jzk, tx with timestamp, for fabric 1.4.8-enhanced
	opt := fab.WithTxTimeStamp(rc.enableTxTimeStamp)

	txID, err := txn.NewHeader(rc.ctx, channelID, opt)
	if err != nil {
...
func (rc *Client) getChannelConfig(channelID string, opts requestOptions) (*chconfig.ChannelConfig, error) {
	...
	return chconfig.New(channelID, rc.enableTxTimeStamp, chconfig.WithOrderer(orderer)) // jzk, tx with timestamp, for fabric 1.4.8-enhanced
}
...
func (rc *Client) requestOrderer(opts *requestOptions, channelID string) (fab.Orderer, error) {
	...
	orderer, err := rc.ctx.InfraProvider().CreateOrdererFromConfig(rc.ctx.InfraProvider().IsGMTLS(), ordererCfg)
	if err != nil {
