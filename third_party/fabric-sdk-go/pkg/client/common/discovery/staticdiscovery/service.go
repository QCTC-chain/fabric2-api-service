...
func NewService(isGMTLS bool, config fab.EndpointConfig, peerCreator peerCreator, channelID string) (*DiscoveryService, error) {
	if channelID == "" {
	...
	for _, p := range chPeers {
		...
		newPeer, err := peerCreator.CreatePeerFromConfig(isGMTLS, &p.NetworkPeer)
