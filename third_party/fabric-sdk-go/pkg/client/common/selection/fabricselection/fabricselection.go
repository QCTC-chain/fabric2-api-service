...
type DiscoveryClient interface {
	...
	Send(isGMTLS bool, ctx context.Context, req *fabdiscovery.Request, targets ...fab.PeerConfig) (<-chan fabdiscovery.Response, error)
}
...
func (s *Service) query(req *fabdiscovery.Request, chaincodes []*fab.ChaincodeCall, targets []fab.PeerConfig) (discclient.ChannelResponse, error) {
	...
	defer cancel()
	//modify by liuhy for gm tls
	responses, err := s.discClient.Send(s.ctx.InfraProvider().IsGMTLS(),reqCtx, req, targets...)
...
func asPeer(ctx contextAPI.Client, endpoint *discclient.Peer) (fab.Peer, error) {
	...
	//modify by liuhy for gm tls
	peer, err := ctx.InfraProvider().CreatePeerFromConfig(ctx.InfraProvider().IsGMTLS(), &fab.NetworkPeer{
