...
func New(channelProvider context.ChannelProvider, opts ...ClientOption) (*Client, error) {
	...
	ledgerClient.enableTxTimeStamp = ledgerClient.enableTxTimeStamp || channelContext.InfraProvider().EnableTxTimeStamp()
	ledgerClient.ledger.EnableTxTimeStamp = ledgerClient.enableTxTimeStamp
...
func (c *Client) QueryConfig(options ...RequestOption) (fab.ChannelCfg, error) {
	...
	channelConfig, err := chconfig.New(c.ctx.ChannelID(), c.enableTxTimeStamp, chconfig.WithPeers(targets), chconfig.WithMinResponses(opts.MinTargets)) // jzk, tx with timestamp, for fabric 1.4.8-enhanced
	if err != nil {
