...
type cacheOptions struct {
	enableTxTimeStamp bool
}
func WithTxTimeStamp(enable bool) options.Opt {
	return func(p options.Params) {
		if opt, ok := p.(cacheOptions); ok {
			opt.enableTxTimeStamp = enable
		}
	}
}
...
func newContextCache(ctx fab.ClientContext, opts []options.Opt) *contextCache {
	...
	options.Apply(&cacheOpt, opts)
	c.enableTxTimeStamp = cacheOpt.enableTxTimeStamp
...
func (c *contextCache) createDiscoveryService(chConfig fab.ChannelCfg, opts ...options.Opt) (fab.DiscoveryService, error) {
	...
	return staticdiscovery.NewService(c.ctx.InfraProvider().IsGMTLS(), c.ctx.EndpointConfig(), c.ctx.InfraProvider(), chConfig.ID())
}
...
func (c *contextCache) loadChannelCfgRef(channelID string) (*chconfig.Ref, error) {
	key, err := chconfig.NewCacheKey(c.ctx, func(string) (fab.ChannelConfig, error) { return chconfig.New(channelID,c.enableTxTimeStamp) }, channelID) // jzk, tx with timestamp, for fabric 1.4.8-enhanced
	if err != nil {
		return nil, err
	}
