...
`dev-1.2`分支改动记录
  - 截至 96b9dc40：
    ...
    - 国密改造
    - 添加submodule
    - 适配cfca证书、jar包、单文件.go
  - 修复内部test包三方引用replace失效问题
  - 同步dev中部分改动，主要为因复杂网络适配的改动，如下
    - 8bbe0247   fix: fix err for mac os
    - e1535d98   fix conn cache for servername
    - f80be9ba   modify entityMatchers , fix scheme missing
  - 内部测试代码，修复接口参数中gm缺失
  - 添加环境变量 TXID_WITH_TIMESTAMP，用作txid生成时的判断条件，`全局唯一`
  - ssdk实例添加参数 enableTxTimeStamp，用作txid生成时的判断条件，通过opts设置`fabsdk.WithTxTimeStamp()`设置，sdk实例内唯一
  - sdk实例是否国密，由fabsdk.New()时bool参数设置，改为fabsdk.New()时opts `fabsdk.WithGMTLS()`设置，sdk实例内唯一
  - 所适配的fabric版本为 `1.4.8`，`1.4.8-enhanced`，`2.2`
