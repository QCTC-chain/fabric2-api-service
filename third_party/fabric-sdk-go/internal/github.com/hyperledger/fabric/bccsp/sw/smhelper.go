...
func CreateCertificateToMem(template, parent *gmx509.Certificate, key bccsp.Key) (cert []byte, err error) {
	...
	case *sm2.PublicKey:
		...
		// puk.X = pub.X
		// puk.Y = pub.Y
		cert, err = gmx509.CreateCertificateToMem(template, parent, pub, pk)
		return
	case *ecdsa.PublicKey:
		//add by liuhy
		pub := template.PublicKey.(*ecdsa.PublicKey)
		der, err := gmx509.CreateCertificate(rand.Reader, template, parent, pub, pk)
		if err != nil {
			return nil, err
		}
		block := &pem.Block{
	...
	default:
		err = fmt.Errorf("%s", "key not unknow publicKey key")
	}
	return
}
...
func CreateSm2CertificateRequestToMem(certificateRequest *gmx509.CertificateRequest, key bccsp.Key) (csr []byte, err error) {
	pk := key.(*sm2PrivateKey).privKey
	csr, err = gmx509.CreateCertificateRequestToMem2(certificateRequest, pk)
	return
}
